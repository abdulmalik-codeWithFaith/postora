...
export default function SignupPage() {
  ...
  // ── Helper: send welcome + verification email via your API route ──────────
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // 3. Get Firebase verification link (sends its own email too — we override below)
      //    We send the email ourselves via Nodemailer, so use sendEmailVerification
      //    just to get a link via actionCodeSettings, OR call it and also send ours.
      //    Simplest: send Firebase verification email to get the link, and send ours separately.
      await sendEmailVerification(user, {
        url: `${process.env.NEXT_PUBLIC_APP_URL}/dashboard`, // redirect after verification
      });

      // 4. Send your custom welcome email (with the Firebase verification link embedded)
      //    Since Firebase doesn't expose the raw link directly from the client SDK,
      //    we send our styled email pointing to a custom verify page, or just tell the
      //    user to check their inbox for both. For a fully custom link, use Firebase Admin
      //    SDK server-side (see note below).
      await sendWelcomeEmail(
        form.email,
        form.name,
        `${process.env.NEXT_PUBLIC_APP_URL}/verify-email?email=${encodeURIComponent(form.email)}`
      );

      router.push("/login"); // show "check your inbox" page
    } catch (err: any) {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (isNew) {
        // Send welcome email for new Google users (email is already verified via Google)
        await sendWelcomeEmail(
          user.email!,
          user.displayName ?? "",
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            cursor: googleLoading ? "not-allowed" : "pointer", opacity: googleLoading ? 0.7 : 1, transition: "border-color 0.2s, background 0.2s",
          }}
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", marginTop: 2 }}>
            <div onClick={() => setAgreed(!agreed)} style={{ width: 18, height: 18, borderRadius: 5, flexShrink: 0, marginTop: 1, background: agreed ? "var(--green)" : "var(--surface-3)", border: `1px solid ${agreed ? "var(--green)" : "var(--border-hover)"}`, display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.2s, border-color 0.2s", cursor: "pointer" }}>
...
// ── All your existing helper components unchanged below ───────────────────────
...
function GoogleIcon() { /* same as before */ return <svg width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>; }
...
function parseFirebaseError(code: string): string {
  switch (code) {
    ...
    case "auth/popup-closed-by-user":
      return ""; // silent — user dismissed the popup
    default:

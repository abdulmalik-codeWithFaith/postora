...
// Posts written by the campaign generation route include `campaignId`, and
// once approved, `locked`. These aren't necessarily in your shared Post type
// yet — extending locally rather than assuming @/types has been updated.
type PostWithCampaign = Post & { campaignId?: string; locked?: boolean };
...
export default function CalendarPage() {
  ...
  // ── Pending-approval campaign detection ───────────────────────────────────
  // Prefer the ?campaign= id from the wizard's redirect; fall back to
  // scanning for any generated-but-not-yet-approved campaign so the banner
  // still shows up if the user navigates back to Calendar later without that
  // query param.
  const queryCampaignId = searchParams.get("campaign");
  const typedPosts = posts as PostWithCampaign[];

  const pendingCampaignId = queryCampaignId
    ?? typedPosts.find((p) => p.campaignId && !p.locked)?.campaignId
    ?? null;

  const campaignPosts = pendingCampaignId
    ? typedPosts.filter((p) => p.campaignId === pendingCampaignId)
    : [];

  const campaignIsLocked = campaignPosts.length > 0 && campaignPosts.every((p) => p.locked);
  const showApprovalBanner = campaignPosts.length > 0 && !campaignIsLocked;

  // ── Approve & lock a campaign ─────────────────────────────────────────────
  // Locks every post generated by this campaign so it can no longer be
  // edited, its status changed, or deleted — the "approve & publish" step
  // from the product flow. Uses a batch so it's all-or-nothing rather than
  // risking half the month getting locked and half not.
  async function handleApprove() {
    if (!pendingCampaignId || campaignPosts.length === 0) return;
    ...
    try {
      ...
      campaignPosts.forEach((p) => {
        batch.update(doc(db, "posts", p.id), { locked: true });
      });
      batch.update(doc(db, "campaigns", pendingCampaignId), {
        approved: true,
      ...
      setPosts((prev) =>
        prev.map((p) =>
          (p as PostWithCampaign).campaignId === pendingCampaignId
            ? ({ ...p, locked: true } as Post)
            : p
        )
      );
      ...
      setApproveError("Couldn't approve the campaign. Please try again.");
    } finally {
      setApproving(false);
    }
  }
  ...
  return (
    <>
      ...
      <main style={{ padding: 28, display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {/* ── Approve & lock banner — only while a generated campaign is
          ...
          <div style={{
            ...
            <div>
              <p style={{ fontSize: 14, fontWeight: 700, color: "var(--text-1)", fontFamily: "var(--font-sora), sans-serif", marginBottom: 3 }}>
                {campaignPosts.length} posts ready for review
              </p>
              <p style={{ fontSize: 12, color: "var(--text-3)" }}>
                Edit any post below if needed. Once you approve, this campaign's posts are locked in and can't be edited or deleted.
              </p>
              {approveError && (
                <p style={{ fontSize: 12, color: "#e24b4a", marginTop: 6 }}>{approveError}</p>
              )}
            </div>
            <button
              ...
              style={{
                ...
                background: "var(--green)", border: "none", borderRadius: 11,
                fontSize: 13, fontWeight: 700, color: "#0a0e14",
        ...
        {justApproved && !showApprovalBanner && (
          <div style={{ padding: "12px 18px", background: "rgba(0,201,141,0.08)", border: "1px solid rgba(0,201,141,0.2)", borderRadius: 12, fontSize: 13, color: "var(--green)" }}>
            ✓ Campaign approved — these posts are now locked in.
          </div>
        )}
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                          ...
                          {(post as PostWithCampaign).locked && <LockIcon size={10} />}
                        </div>
      ...
      {selectedPost && (
        <PostModal
          post={selectedPost}
          locked={(selectedPost as PostWithCampaign).locked === true}
          deleting={deleting}
          saving={saving}
          saveError={saveError}
...
// ─────────────────────────────────────────────────────────────────────────────
function PostModal({
  post, locked, deleting, saving, saveError, onClose, onDelete, onSave,
}: {
  post: Post;
  locked: boolean;
  deleting: boolean;
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, backdropFilter: "blur(4px)" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--surface-2)", border: "1px solid var(--border-hover)", borderRadius: 20, width: "100%", maxWidth: 440, overflow: "hidden" }}>
        ...
        <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {locked && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "var(--surface-3)", border: "1px solid var(--border)", borderRadius: 10, fontSize: 12, color: "var(--text-3)" }}>
              <LockIcon /> This post is part of an approved campaign and can't be edited.
            </div>
          )}
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <select
              ...
              style={{ padding: "6px 12px", background: "var(--surface-3)", border: "1px solid var(--border-hover)", borderRadius: 9, fontSize: 12, color: "var(--text-1)", outline: "none", colorScheme: "dark", cursor: (editing || locked) ? "not-allowed" : "pointer", opacity: (editing || locked) ? 0.5 : 1 }}
            >
            ...
            {editing ? (
              ...
              <>
                <button
                  ...
                  disabled={locked}
                  style={{ flex: 1, padding: "11px", background: locked ? "var(--surface-4)" : "var(--green)", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, color: locked ? "var(--text-3)" : "#0a0e14", cursor: locked ? "not-allowed" : "pointer" }}
                >
                  {locked ? "Locked" : "Edit post"}
                </button>
                <button
                  onClick={onDelete}
                  disabled={deleting || locked}
                  style={{ padding: "11px 16px", background: "rgba(226,75,74,0.1)", border: "1px solid rgba(226,75,74,0.2)", borderRadius: 10, fontSize: 13, fontWeight: 500, color: "#e24b4a", cursor: (deleting || locked) ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 6, opacity: (deleting || locked) ? 0.4 : 1 }}
                >
...
function LockIcon({ size = 14 }: { size?: number }) { return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="var(--text-3)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>; }

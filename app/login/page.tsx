...
export default function LoginPage() {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      // Execution stops here — result handled in useEffect above on return
    } catch (err: any) {
      setError(parseFirebaseError(err.code));
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--surface)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 16px" }}>
          ...
          <div style={{ padding: "12px 14px", background: "rgba(226,75,74,0.12)", border: "1px solid rgba(226,75,74,0.3)", borderRadius: 10, fontSize: 13, color: "#e24b4a", display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {unverified && (
              resendSent
                ...
                : <button onClick={handleResend} style={{ alignSelf: "flex-start", background: "none", border: "none", cursor: "pointer", color: "#ef9f27", fontSize: 12, fontWeight: 600, padding: 0, textDecoration: "underline" }}>Resend verification email</button>
            )}
          </div>
        )}
